...
| STU-FR-003 | MUST | Danh sách học liệu chỉ chứa publication active; PPTX xếp trước PDF/DOCX. |
| STU-FR-004 | MUST | PPTX READY được xem theo slide trên Web, không tải file gốc. |
| STU-FR-005 | MUST | PDF/DOCX public được tải xuống sau khi Java kiểm quyền. |
| STU-FR-006 | MUST | Student lưu một Note riêng cho từng document/slide. |
| STU-FR-007 | MUST | Slide Tutor trả lời kèm citation thuộc PPTX được phép. |
| STU-BR-001 | MUST | PDF/DOCX Teacher không có viewer, Note, Tutor hoặc page progress. |
...
| `GET /api/v1/student/materials/{id}/download` | PDF/DOCX public | redirect/signed URL ngắn hạn | `403 FILE_TYPE_NOT_DOWNLOADABLE`, `404` |
...
- **Given** một ClassSubject có PPTX, PDF và DOCX đã public
- **When** Student thuộc lớp mở danh sách học liệu
- **Then** PPTX có Viewer/Note/Tutor nhưng không download; PDF và DOCX chỉ có download.
